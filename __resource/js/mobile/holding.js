...
const HoldingSearch = class {
    constructor() {
        ...
        // this.fieldDetailElement = findOne('.holding-search__field-detail');

        if (!this.element) return;

        this.headerHeight = findOne('.site-header').clientHeight;

        this.formButton = findOne('.holding-search > button', this.element);
        this.formSerch = findOne('.holding-search__form', this.element);
        this.formInputs = [
            ...find('input:not([type="hidden"]), select', this.formSerch),
            ...find('[name$="appraisal_total_amt"]', this.formSerch)
        ];
        this.formDetail = findOne('.holding-search__field-detail', this.formSerch);
    ...
    resetForm() {
        ...
        this.toggleForm('remove');
        this.formButton.classList.remove('active');
        this.formDetail.classList.remove('holding-search__field-detail--open')
        this.clearGugun();
        this.clearLocation();
    ...
    toggleForm(action) {
        const active = 'active';
        const activeClass = 'holding-search__form--open';

        if (action && ['add', 'remove'].includes(action)) {
            this.formSerch.classList[action](activeClass);
            this.formButton.classList.add(active);
        } else {
            this.formSerch.classList.toggle(activeClass);
            this.formButton.classList.toggle(active);
        }

        if (!this.formSerch.classList.contains(activeClass)) {
            window.scroll({
    ...
    toggleDetail(action2) {
        const activeClass = 'holding-search__field-detail--open';

        if (action2 && ['add', 'remove'].includes(action2)) {
            this.formDetail.classList.add(activeClass);
        } else {
        ...
        if (!this.formDetail.classList.contains(activeClass)) {
            window.scroll({
                top: getOffset(this.formDetail).top - this.headerHeight,
                behavior: 'smooth'
            });
        }
    }
    ...
    toggleDetailClose() {
        const activeClass = 'holding-search__field-detail--open';
        if (this.formDetail.classList.contains(activeClass)) {
            this.formDetail.classList.remove(activeClass);
        }
    ...
    initEvents() {
        ...
        (() => {
            const searchDetailTrigger = findOne('.holding-search > button', this.element);

            on(searchDetailTrigger, 'click', this.toggleForm.bind(this));
        })();

        (() => {
            const detailTrigger = findOne('.holding-search__field-detail-title', this.element);

            on(detailTrigger, 'click', this.toggleDetail.bind(this));
        })();

        (() => {
            const detailTriggerClose = findOne('.holding-search__field-detail--close', this.element);

            on(detailTriggerClose, 'click', this.toggleDetailClose.bind(this));
        })();

        (() => {
            const formSearchTrigger = findOne('.holding-search__button--search');

            on(formSearchTrigger, 'click', this.toggleForm.bind(this));
        })();
